	...
	<view class="auth-page">
		<view class="auth-card">
			...
			<view class="auth-logo">
				...
				<text class="logo-name">ChatAPP</text>
				<text class="logo-subtitle">你的对话，只属于你</text>
			</view>
...
import { setTokens, getServerConfig, buildUrl, isLoggedIn } from '@/utils/constants.js';
	...
	try {
			...
			uni.request({
				...
				success: (res) => {
					if (res.statusCode === 200 && res.data && res.data.code === 0) {
						...
						const msg = (res.data && res.data.detail && res.data.detail.message) || (res.data && res.data.message) || '登录失败';
		...
		setTimeout(() => {
			if (redirectUrl.value) {
				// 有重定向目标时使用 navigateTo/redirectTo
